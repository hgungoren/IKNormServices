...
namespace Serendip.IK.Nodes
{
    public class NodeAppService : IKCoreAppService<Node, NodeDto, long, PagedNodeRequestDto, NodeCreateInput, NodeUpdateInput>, INodeAppService
    {
        ...
        public async Task<bool> UpdateSelected(ChangeSelectedDto changeSelectedDto)
        {
            var node = await Repository.FirstOrDefaultAsync(x => x.PositionId == changeSelectedDto.PositionId && x.Id == changeSelectedDto.Id);
            node.Selected = changeSelectedDto.Selected;
            await Repository.UpdateAsync(node);
            return changeSelectedDto.Selected;
        }
